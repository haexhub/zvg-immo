...
>
> **Neu: Geocoding-Beobachtbarkeit.** `auctions` hat jetzt `geocode_attempted_at`/`geocode_result`/`geocode_provider` (Migration `0004_nappy_vengeance.sql`), verdrahtet in `server/tasks/geocode.ts` (schreibt für jede versuchte Adresse, unabhängig vom Ergebnis) über eine neue `recordGeocodeAttempts()` in `current-auctions.ts`. Geschrieben wird in 500er-Blöcken während des Laufs plus einem Abschluss-Flush im `finally` — ein Kaltlauf dauert ~40 min und wird von `runExclusiveTask` abgebrochen, sobald ein neuerer Lauf startet; ein einziger Schreibvorgang am Ende hätte die Beobachtbarkeit genau für die langen Läufe verloren, für die sie existiert. Nicht erfasst sind Auktionen **ohne** Adresse (die Schleife läuft nur über `withAddress`) — die bleiben auf „nie versucht" und sind über `address IS NULL` ohnehin direkt abfragbar. Gegen echtes lokales Postgres getestet (docker-compose `db`+`auth`, `drizzle-kit migrate`).
