...
Stand: 2026-08-08 · Status: geplant, nichts implementiert
...
| Kein Modell/Profil wird pro Version gespeichert — nur `extraction_source` ('rules'/'llm'), `extraction_confidence`, `llm_analyzed_at` | Ohne neue Spalten sind zwei Versionen keinem Modell zuzuordnen. |
| Vier Lesepfade wählen „die aktuelle Version" per `ORDER BY version DESC LIMIT 1` statt per `is_latest`: `server/utils/auction-record.ts:130`, `server/utils/auction-details.ts:257`, `server/api/auctions.get.ts:77`, `server/utils/artifact-version-state.ts:62` | Trial-Versionen würden sofort öffentlich durchschlagen. Muss zuerst repariert werden. |
...
| `task_run_errors` speichert Fehler pro Auktion, aber nur `enrich.ts` schreibt dorthin; `reprocess.ts` loggt nur nach stdout (`server/tasks/reprocess.ts:878`) | „Wo gab es Fehler" ist für den gesamten LLM-Zweig aktuell nicht beantwortbar. |

## WP-0 · Trial-Versionen tragfähig machen

Fundament, muss vor WP-4 liegen.

1. Migration: `auction_details.is_trial boolean not null default false`.
2. Die vier Lesepfade oben auf `WHERE ad.is_latest` umstellen. Nutzt den
...
3. `writeAuctionDetails` (`server/utils/auction-details.ts:305`) bekommt
   `options.trial`:
   ...
   - Die „previous latest"-Ermittlung für normale Schreibvorgänge muss von
     `max(version)` auf `is_latest` umgestellt werden, sonst degradiert ein
     Cron-Lauf eine Trial-Zeile statt der Live-Zeile.

**Verifizieren:** Test in `server/utils/auction-details.test.ts`: Trial schreiben
→ `readLatestAuctionDetails` liefert weiterhin die alte Live-Version; danach
normaler Lauf → Version zählt korrekt weiter, `is_latest` bleibt eindeutig.

**Risiko:** Bleibt ein Lesepfad auf `ORDER BY version DESC`, wird das erste
Experiment still öffentlich sichtbar. Pro Lesepfad ein expliziter Test, bevor
WP-4 gebaut wird.

## WP-1 · Provenienz pro Version

Migration + Schreibpfad: `llm_provider`, `llm_model`, `llm_profile_id`,
`run_trigger` (`'cron' | 'manual'`), optional `llm_duration_ms` auf
`auction_details`.

Bewusst **nicht** in `VALUE_COLUMNS` aufnehmen — sonst mintet ein reiner
Modellwechsel bei identischen Fakten eine Version. Durchreichen wie
...
`server/tasks/reprocess.ts:489` weiß, welcher Config gewonnen hat).

**Verifizieren:** `pnpm test` plus ein scoped `reprocess`-Lauf gegen eine
Auktion → neue Version trägt Provider/Modell.
...
- **Löschen:** `DELETE …/versions/[version]` — verweigert wenn `is_latest`
  (erst promoten, dann löschen). Cascades sind bereits korrekt:
  `auction_photos` über `auction_details_id`, `auction_translations` über die
  zusammengesetzte FK (`server/db/schema/translations.ts:62`). Mehrfachauswahl
...
- Extraktionshinweis „…automatisch extrahiert (hohe/niedrige Konfidenz)" in
  `components/Auction/DetailOverviewSections.vue:362`
...
`runReprocess` schreibt im `catch`-Block (`server/tasks/reprocess.ts:878`)
...
```
WP-0 (Trial-Fundament)  ─┐
WP-1 (Provenienz)       ─┤→ WP-2 (API) → WP-3 (Seite) → WP-4 (Einzellauf) → WP-5 (Diff/Promote/Delete)
WP-7 (Fehler-Logging)   ─┘
WP-6 (Public-Cleanup)   ── unabhängig, jederzeit
```

WP-0, WP-1, WP-7 und WP-6 sind je ein kleiner, unabhängig mergebarer PR.
WP-2+3 gehören in einen PR (API ohne UI bringt nichts), WP-4 und WP-5 je einer.
Realistisch sechs PRs.
