...
> **Status 2026-08-05: erledigt (PR #318).** Der Aufbau-Job lebt in [server/tasks/build-geo-features.ts](../../server/tasks/build-geo-features.ts), das vollständige Kind-Mapping (alle 14 `kind`s aus der Tabelle unten, inklusive der bis WP-6 leeren) ist umgesetzt, gegen ein lokales Postgres+PostGIS verifiziert (Kind-Zuordnung, Lake/River-Ausschluss, Idempotenz über zwei Läufe, Skip-and-log bei kaputter Geometrie, KNN-Index-Nutzung per `EXPLAIN`). **Noch nicht gegen Prod gelaufen**: `osm_local_elements` ist dort seit dem WP-0-Neuaufbau leer (0 Zeilen laut `pg_stat_user_tables`, Gesamt-DB nur noch 168 MB statt der ursprünglichen ~20 GB) — der externe osm2pgsql-Reimport steht noch aus, ein Lauf auf Prod würde `geo_features` heute mit nichts befüllen. Der `geometry(Geometry, srid)`-`customType` aus dem WP-2-Verweis unten existiert bereits — WP-2 ist obsolet, der `customType` lebt jetzt in [server/db/schema/geo.ts](server/db/schema/geo.ts). **`geo_features` und `auction_geo_metrics` sind als leere Skeleton-Tabellen bereits angelegt** (Migration aus WP-0). Die tatsächlichen Spalten weichen leicht vom Sketch unten ab:
...
- **Advisory-Lock über den gesamten Aufbau** (`pg_try_advisory_lock`, Session-Level auf dem Job-Client). `runExclusiveTask` serialisiert nur innerhalb *eines* Node-Prozesses — zwei Container (oder ein Container plus ein manuell getriggerter Ad-hoc-Lauf) würden sonst dieselbe `max(features_epoch)+1` lesen, unter derselben Epoch schreiben und sich anschließend gegenseitig die Zeilen löschen. Bewusst kein Transaktions-Lock: eine einzige Transaktion über den 44,5-Mio.-Zeilen-Scan widerspricht dem Per-`kind`-Commit oben und würde stundenlang WAL festhalten.
- **Kein Fallback bei systemischen Fehlern.** Schlägt der Bulk-INSERT eines `kind` fehl, isoliert ein Per-Zeile-Durchlauf die eine kaputte Geometrie. Das gilt aber nur für zeilenlokale Fehler: bei systemischen SQLSTATE-Klassen (42 Schema/Rechte, 57 Timeout, 53 Ressourcen, 08 Verbindung, …) — und ebenso, wenn im Per-Zeile-Durchlauf *jede* Zeile scheitert — wird der Fehler durchgereicht. Sonst meldet der Lauf einen erfolgreichen Rebuild mit lauter „skipped" und löscht anschließend die noch guten Zeilen der alten Epoch.
