...
<template>
  <Card>
    <CardHeader>
      <CardTitle>{{ $t('settings.crawlStatus.title') }}</CardTitle>
      <CardAction>
        <Button type="button" variant="ghost" size="icon-sm" :disabled="pending" :title="$t('settings.crawlStatus.refresh')" @click="load">
    ...
    <CardContent class="space-y-4">
      <p class="text-sm text-muted-foreground">{{ $t('settings.crawlStatus.description') }}</p>
      <p v-if="error" class="text-sm text-destructive">{{ error }}</p>
      <p v-if="crawlAction.error.value" class="text-sm text-destructive">{{ crawlAction.error.value }}</p>
      <p v-if="!pending && rows.length === 0" class="text-sm text-muted-foreground">{{ $t('settings.crawlStatus.empty') }}</p>
      ...
      <div class="divide-y rounded-md border">
        <template v-for="row in rows" :key="row.code">
          <div class="flex flex-wrap items-center justify-between gap-3 px-3 py-2.5">
            ...
            />
            <Button type="button" variant="outline" size="sm" :disabled="crawlAction.pending.value" @click="retryCountry(row.code)">
              <Loader2 v-if="crawlAction.pending.value" class="h-4 w-4 animate-spin" />
              <RefreshCw v-else class="h-4 w-4" />
              {{ $t('settings.crawlStatus.retry') }}
            </Button>
          </div>
