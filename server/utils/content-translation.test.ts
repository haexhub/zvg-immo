...
function makeFakePool() {
  const rows = new Map<string, { title: string | null; description: string | null }>()

  const query = vi.fn(async (sql: string, params: unknown[] = []) => {
    if (sql.includes('SELECT title, description FROM content_translations')) {
      const [hash, lang] = params as [string, string]
      const hit = rows.get(`${hash}:${lang}`)
      return { rows: hit ? [hit] : [] }
    }
    if (sql.includes('INSERT INTO content_translations')) {
      const [hash, lang, title, description] = params as [string, string, string | null, string | null]
      const key = `${hash}:${lang}`
      if (!rows.has(key)) rows.set(key, { title, description })
...
describe('readContentTranslation / writeContentTranslation', () => {
  ...
  it('returns the written row on a cache hit', async () => {
    ...
    await writeContentTranslation(db, 'hash-a', 'en', 'Title EN', 'Description EN')
    const hit = await readContentTranslation(db, 'hash-a', 'en')
    expect(hit).toEqual({ title: 'Title EN', description: 'Description EN' })
  })
  ...
  it('is immutable per (content_hash, lang) — a second write does not overwrite', async () => {
    const pool = makeFakePool()
    const db = { query: pool.query } as Pool
    await writeContentTranslation(db, 'hash-a', 'en', 'First', 'First desc')
    await writeContentTranslation(db, 'hash-a', 'en', 'Second', 'Second desc')
    const hit = await readContentTranslation(db, 'hash-a', 'en')
    expect(hit).toEqual({ title: 'First', description: 'First desc' })
  })

  it('a changed content_hash is a distinct cache entry (content change -> new translation)', async () => {
    const pool = makeFakePool()
    const db = { query: pool.query } as Pool
    await writeContentTranslation(db, 'hash-a', 'en', 'Old title', 'Old desc')
