...
import { proximityCondition, proximityConditionAnyOf, proximityConditionAnyTag } from './osm-proximity'
...
describe('osm-proximity', () => {
  ...
  it('proximityCondition references a.lat/a.lng, not d.lat/d.lng', () => {
    const sql = proximityCondition('natural', 'peak', 5000, add)
  ...
  it('proximityConditionAnyTag references a.lat/a.lng, not d.lat/d.lng', () => {
    const sql = proximityConditionAnyTag(
      [{ tagKey: 'natural', tagValue: 'coastline' }, { tagKey: 'water', tagValue: 'sea' }],
      5000,
      add,
    )
    expect(sql).toContain('a.lat')
