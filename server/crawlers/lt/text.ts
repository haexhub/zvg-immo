 ...
 *  "2,5 ha" → 25000. Large values group thousands with spaces ("1 234,56"). */
export function parseLtArea(raw: string): number | null {
  const num = (s: string) => parseFloat(s.replace(/\s/g, '').replace(',', '.'))
  const n = '(\\d+(?:\\s\\d{3})*(?:[.,]\\d+)?)'
  const kv = raw.match(new RegExp(`${n}\\s*kv\\.?\\s*m`, 'i'))
  if (kv?.[1]) return num(kv[1])
  const ar = raw.match(new RegExp(`${n}\\s*a\\b`, 'i'))
  if (ar?.[1]) return num(ar[1]) * 100
  const ha = raw.match(new RegExp(`${n}\\s*ha\\b`, 'i'))
