// Fires llm-batch-poll once shortly after server startup so a Gemini Batch
// job that finished while the server was down/restarting gets merged
...
// never crawls an upstream portal (just Postgres + the Gemini Batch API), so
