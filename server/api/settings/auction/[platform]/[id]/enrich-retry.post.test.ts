...
async function loadHandler() {
  ...
  return (await import('./enrich-retry.post')).default as unknown as (event: unknown) => Promise<unknown>
}
...
describe('/api/settings/auction/[platform]/[id]/enrich-retry', () => {
  ...
  it('triggers a scoped enrich retry for exactly this auction', async () => {
    vi.stubGlobal('getRouterParam', (_e: unknown, name: string) => (name === 'platform' ? 'zvg-portal' : '7265'))
    const { readAuctionRecord } = await import('~/server/utils/auction-record')
