...
export default defineEventHandler(async (event): Promise<CrawlStatusList> => {
  ...
  if (sort && !['platform', 'title', 'region', 'error'].includes(sort)) {
  ...
  return readCrawlStatusList(country, bucket, search || sort
    ? { limit, offset, search, sort: sort as 'platform' | 'title' | 'region' | 'error' | undefined, direction: direction as 'asc' | 'desc' }
    : { limit, offset })
})
